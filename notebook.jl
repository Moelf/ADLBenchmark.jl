...
# ╔═╡ d282e4f7-fc0b-42b7-a69f-934e53c5698d
using UnROOT, FHist, CairoMakie, LorentzVectorHEP, LazyArrays

# ╔═╡ f4f9aa5d-7738-40bf-a8ea-0260c70399f1
using Combinatorics: combinations,Combinations

# ╔═╡ f003ab82-64fc-4fb1-95f3-e599d7154ecc
Threads.nthreads()

# ╔═╡ 5c1ecc37-e802-46ab-9fca-c3a784484f72
CairoMakie.activate!(; px_per_unit=2) #hide

# ╔═╡ 4abbd5c5-816d-48ad-a4cb-7c72f4823143
const mytree = LazyTree("/home/akako/Downloads/Run2012B_SingleMu.root", "Events", r"MET|Muon|Jet|Electron");

# ╔═╡ b62d9199-6239-4630-828d-0e2fabb6e658
md"## Query 1
Plot the missing ET of all events.
"

# ╔═╡ 8fb0ebb7-4b36-4c42-83e3-360f99c6034e
function Q1(tree)
	Hist1D(tree.MET_pt, 0:2:200)
end

# ╔═╡ 03c5d55e-2572-4356-924e-ef95c4361e41
q1_result = @time Q1(mytree)

# ╔═╡ 58422fbb-0524-4d02-98b2-57604d15e842
stairs(q1_result; axis=(xticks=0:25:200, xlabel=L"E_T^\text{Miss} \text{[GeV]}"))

# ╔═╡ 9008b659-021b-4e34-9cd4-25545dafd9ff
md"## Query 2
Plot pT of all jets in all events.
"

# ╔═╡ 967ae418-feec-48f3-9153-7bf1cab1e596
function Q2(tree)
	h = Hist1D(; bins = 0:2:200)
	for pts in tree.Jet_pt, pt in pts
		push!(h, pt)
	end
	return h
end

# ╔═╡ 6d4b684e-6583-42b2-a3d5-b5ab470f8cce
q2_result = @time Q2(mytree);

# ╔═╡ fc1f69bd-7a91-4c8f-b0c1-56d37d2a7d47
stairs(q2_result; axis=(xticks=0:25:200, xlabel=L"\text{Jet } p_{T} \text{ [GeV]}"))

# ╔═╡ 7ffd6d62-3f0b-4cd1-bee7-d741e8cf2a79
md"## Query 3
Plot pT of jets with |η| < 1.
"

# ╔═╡ a884cee2-0d1d-4290-b45f-ec04927a9331
function Q3(tree)
	h = Hist1D(; bins = 0:2:200)
	for evt in tree
		(; Jet_pt, Jet_eta) = evt
...
# ╔═╡ d2e68d62-7407-488d-9e26-89dba344553e
q3_result = @time Q3(mytree);

# ╔═╡ 57131cf5-8326-4b00-ae73-9436f85c042e
stairs(q3_result; axis=(xticks=0:25:200, xlabel=L"\text{Jet } p_{T} \text{ [GeV]}"))

# ╔═╡ 5475ef10-4d03-442f-9a6c-29801bc9b27d
md"## Query 4
Plot the missing ET of events that have at least two jets with pT > 40 GeV.
"

# ╔═╡ c01d2e8b-588b-4101-b3ec-2e879467eee0
function Q4(tree)
	h = Hist1D(; bins = 0:2:200)
	for evt in tree
...
# ╔═╡ 3581000d-5dc1-453d-8c1d-094e3e1c7c16
q4_result = @time Q4(mytree);

# ╔═╡ 59acdb46-906f-4af3-b7c9-5c0c3336c62c
stairs(q4_result; axis=(xticks=0:25:200, xlabel=L"E_T^\text{Miss} \text{[GeV]}"))

# ╔═╡ 01b3c0f3-2ef9-4fef-a68e-c7e4924909d0
md"## Query 5
Plot the missing ET of events that have an opposite-sign muon pair with an invariant mass between 60 and 120 GeV.
"

# ╔═╡ cd6f3699-4d38-4d0b-b615-51be3526a8c1
function Q5(tree)
	h = Hist1D(Int; bins = 0:2:200)
	for evt in tree
		evt.nMuon < 2 && continue
		(; Muon_pt, Muon_eta, Muon_phi, Muon_mass, Muon_charge) = evt
		goodevent = false
		for i in eachindex(Muon_pt)
			i_charge = Muon_charge[i]
			v4i = LorentzVectorCyl(Muon_pt[i], Muon_eta[i], Muon_phi[i], Muon_mass[i])
			for j in i+1:length(Muon_pt)
				i_charge == Muon_charge[j] && continue
				v4j = LorentzVectorCyl(Muon_pt[j], Muon_eta[j], Muon_phi[j], Muon_mass[j])
				if 60 < fast_mass(v4i, v4j) < 120
					goodevent = true
					break
				end
			end
		end
		goodevent && push!(h, evt.MET_pt)
	end
	return h
end

# ╔═╡ 99deefd3-1ed2-42a6-874c-6b2c167c5c6a
q5_result = @time Q5(mytree);

# ╔═╡ 6798a623-7cd0-4e9f-a127-60afda42dfa3
stairs(q5_result; axis=(xticks=0:25:200, xlabel=L"E_T^\text{Miss} \text{[GeV]}"))

# ╔═╡ 02b796cc-d964-466b-b493-67a02c501c96
md"## Query 6
...
# ╔═╡ 5433a618-eb49-4b05-a5db-1b005b809825
function Q6(tree)
	h = Hist1D(; bins = 0:2:200)
	bh = Hist1D(; bins = 0:0.01:1)
	target = 172.5
	...
	Threads.@threads for evt in tree
		...
		nJet < 3 && continue
		(; Jet_btag) = evt
		jet_4vecs = LorentzVectorCyl.(evt.Jet_pt, evt.Jet_eta, evt.Jet_phi, evt.Jet_mass)
		best_mass = Inf32
		local best_pt
		local bscore
		
		@views for combo in Combinations(nJet, 3)
			(; pt, mass) = sum(jet_4vecs[combo])
			temp_diff = abs(mass - target)
			if  abs(mass - target) < abs(best_mass - target)
				best_pt = pt
				bscore = maximum(Jet_btag[combo])
            end
		end
		atomic_push!(h, best_pt)
		atomic_push!(bh, bscore)
	end
	
	return h, bh
end

# ╔═╡ c9d555a4-2022-43f7-80d1-28d437a53048
q6_result = @time Q6(mytree);

# ╔═╡ 15e2878b-ef3a-4bcf-8d45-3f76b1c95882
begin
	f, a, _ = stairs(q6_result[1];
		axis = (xticks=0:25:200, xlabel = L"\text{Trijet } p_{T} \text{ [GeV]}"),
		figure=(; resolution=(800, 300)))
	stairs(f[1,2], q6_result[2]; axis = (xticks=0:0.2:1, xlabel = "Max jet b-tag score"))
	f
end

# ╔═╡ 256dd45f-dee4-40d4-a745-ad40c1236bd3
md"## Query 7
Plot the sum of pT of jets with pT > 30 GeV that are not within 0.4 in ΔR of any lepton with pT > 10 GeV.
"

# ╔═╡ 9803eaa5-6085-4aad-a39d-106e840f35b8
q7_fail(jet, lep) = pt(lep) > 10 && ΔR(jet, lep) < 0.4

# ╔═╡ a3cd70a8-1536-48ce-89ed-55b552bb3727
function Q7(tree)
	h = Hist1D(; bins = 0:2:200)
	
	Threads.@threads for evt in tree
		s = 0.0
		jets = LorentzVectorCyl.(evt.Jet_pt, evt.Jet_eta, evt.Jet_phi, evt.Jet_mass)
		electrons = LorentzVectorCyl.(evt.Electron_pt, evt.Electron_eta, evt.Electron_phi, evt.Electron_mass)
		muons = LorentzVectorCyl.(evt.Muon_pt, evt.Muon_eta, evt.Muon_phi, evt.Muon_mass)

		for jet in jets
			jet.pt < 30 && continue
			for (e, m) in zip(electrons, muons)
				q7_fail(jet, e) && @goto nextjet
				q7_fail(jet, m) && @goto nextjet
			end
			s += jet.pt
			@label nextjet
		end
		...
		muons = LorentzVectorCyl.(evt.Muon_pt, evt.Muon_eta, evt.Muon_phi, evt.Muon_mass)
		!iszero(s) && atomic_push!(h, s)
	end
	
	return h
end

# ╔═╡ f3d91610-5eb8-4039-8a64-74ffc05a3755
q7_result = @time Q7(mytree);

# ╔═╡ 51ac4e18-8ae2-4ff5-8032-6608260a7326
stairs(q7_result; axis=(xticks=0:25:200, xlabel=L"\sum p_{T} \text{ [GeV]}"))

# ╔═╡ ff65c755-5824-4665-9797-4743651932c4
md"## Query 8
...
# ╔═╡ bd5752dc-4e98-42b0-81eb-307f35f5afb0
function Q8(tree)
	h = Hist1D(; bins = 0:2:200)
	Z_m = 91.2
	Threads.@threads for evt in tree
...
# ╔═╡ 0cf1ccc3-3761-434d-8372-f8690c2f450b
q8_result = @time Q8(mytree);

# ╔═╡ 5a244035-9aaa-42fe-9234-a9f9e9ef4a3d
stairs(q8_result; 
	axis=(xticks=0:25:200, xlabel=L"l-\text{MET transverse mass [GeV]}")
)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "b9ef088c6a16c97430fe7da82eba64eb231896c0"
...
git-tree-sha1 = "6a8017352d4e9a2cac5b228cc09f68dcc0d3409e"
...
[[deps.UnROOT]]
deps = ["AbstractTrees", "ArraysOfArrays", "CodecLz4", "CodecXz", "CodecZstd", "HTTP", "IterTools", "LRUCache", "LibDeflate", "LorentzVectors", "Memoization", "Mixers", "Mmap", "Parameters", "PrettyTables", "SentinelArrays", "StaticArrays", "Tables", "xrootdgo_jll"]
git-tree-sha1 = "2e0ae46bba7c4abb9d6455f817644060713ad6c5"
...
git-tree-sha1 = "e9bd3ead50b5767f7525e4a52dc8e4d90ef65f0a"
